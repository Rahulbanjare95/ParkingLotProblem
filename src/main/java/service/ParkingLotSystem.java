...
public class ParkingLotSystem {
    ...
    public void parkVehicle(Car carDetails, DriverCategory type, String attendant) throws ParkingLotException {
        ...
        for (ParkingLot lot : parkingLotList) {
            ...
            if (isVehiclePresent) throw new AssertionError("Already parked");
        }
        if (carDetails.getType() == DriverCategory.HANDICAP) {
            if (carDetails.getSize().equals(CarSize.SMALL))
            ...
            if (carDetails.getSize().equals(CarSize.LARGE)) {
    ...
    private ParkingLot getPositionOnLot() throws ParkingLotException {
        int totalParked = IntStream.range(0, parkingLot).map(slot -> this.parkingLotList.get(slot)
                .getNumberOfParkedCars()).sum();
    ...
    public List<String> findCarsByRowForHandicap(DriverCategory type, CarSize size) {
    ...
    public List<String> getAllParkedVehicles() {
        ...
        int lot = 0;
        for (ParkingLot parkingLot : parkingLotList) {
            List<String> details = parkingLot.findAllCarsParked();
            lot++;
            parking.add("lot " + lot + "==" + details);
        }
        return parking;
    }
}
